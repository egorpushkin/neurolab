...
int __afxDropDownWidth = -1;

int AFXAPI __AfxGetDropDownWidth()
{
	// return cached version if already determined...
	if (__afxDropDownWidth != -1)
		return __afxDropDownWidth;
	...
	VERIFY(GetCharWidth(hDC, '6', '6', &__afxDropDownWidth));
	if (hFont != NULL)
	{
				...
				else
				{
					...
					else
					{
						...
						if (wStyle == 8) 
						{
							...
							CRect rcCli=rcBtn;
							int dif=rcBtn.right-__afxDropDownWidth;
							rcCli.left=dif+1;
...
void CGuiToolBarWnd::DrawArrow(CDC* pDC,CRect m_rc)
{
	...
	m_rc.left=m_rc.right-__AfxGetDropDownWidth();
...
CSize  CGuiToolBarWnd::CalcSize(TBBUTTON* pData, int nCount)
{
	...
	if (bDrow == TRUE)
	{
		...
		if (bVertDocked)
		{
			sizeResult.cx-=__afxDropDownWidth;
			sizeResult.cy+=__afxDropDownWidth-2;//*nNumDrow;
		}
		else
		{
			sizeResult.cx-=__afxDropDownWidth*nNumDrow;
	...
	{
		if (bVertDocked)
			sizeResult.cy+=__afxDropDownWidth/2;
	}
