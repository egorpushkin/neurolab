...
static void *thread_entry(void *arg) {
	...
	const char* funcName = lua_tostring(st->child, 1);
   	stack_parsesequence(st->child, funcName, true);
    lua_replace(st->child, 1);
	...
	try
	{
		int error = lua_pcall(st->child, n-1, 0, 0);
		if ( error )
		{
			return NULL;
		}
...
static int newthread(lua_State *parent) {
    ...
    st->parent = parent;
    luaL_checktype(st->parent, 1, LUA_TSTRING);
...
static int sleep(lua_State *L)
{
	...
	ulong delay = (ulong)lua_tonumber(L, 1);
...
int luaopen_thread_core(lua_State *L) {
    auxiliar_open(L);
