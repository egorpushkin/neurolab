...
void CInfoBar::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	sCtrlButton* pButton = GetButtonUnderMouse(point);	
	if (pButton != pActiveButton)
	{
		pActiveButton = pButton;
		Invalidate();
	}
...
void CInfoBar::SetToolTipText(CString& text, int iToolID, CString& title)
{
	InitToolTips();
	...
	if (!bToolAdded)
	{
		...
		BOOL bResult = mToolTip.AddTool(this, text, &ClRect, iToolID);

		bToolAdded = true;
	}
	//}

	mToolTip.UpdateTipText(text, this, iToolID);
...
void CInfoBar::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		sCtrlButton* pButton = GetButtonUnderMouse(point);	
		if (pButton && pButton->iCtrlID != -1)	
		{				
			if (pButton->iButtonType == BT_STANDART_BUTTON || 
				pButton->iButtonType == BT_ICON)
			{				
				GetParent()->SendMessage(WM_COMMAND, pButton->iCtrlID, (LPARAM)m_hWnd);		

				pActiveButton = NULL;
				Invalidate();

			} else if (pButton->iButtonType == BT_DROPDOWN_MENU_BUTTON)
			{
				CMenu tmPopup;
				tmPopup.LoadMenu(pButton->iCtrlID);

				CMenu* pPopupMenu = tmPopup.GetSubMenu(0);

				CRect ClRect;
				GetClientRect(&ClRect);

				CPoint tmPosition(GetButtonOffset(pButton), ClRect.Height() - 2);
				ClientToScreen(&tmPosition);

				pPopupMenu->TrackPopupMenu(
					TPM_LEFTALIGN|TPM_LEFTBUTTON,
					tmPosition.x, tmPosition.y, GetParent());				
...
void CInfoBar::SetRightPanelTips(CString& text, int iToolID, int iTTMaxWidth)
{
	if (!bPopupToolTipEnabled)
	{
	...
	if (!bPopupToolAdded)
	{
		...
		mPopupToolTip.AddTool(this, text, &ClRect, iToolID);
		bPopupToolAdded = true;
	}

	mPopupToolTip.UpdateTipText(text, this, iToolID);

	iRightToolCtrlID = iToolID;
}
...
void CInfoBar::UpdateToolTips()
{
	if (bToolAdded)
	{
		...
		mToolTip.SetToolRect(this, iToolCtrlID, &ClRect);
	}
	...
	if (bPopupToolAdded)
	{
		...
		mPopupToolTip.SetToolRect(this, iRightToolCtrlID, &ClRect);
	}
...
BOOL CInfoBar::OnCommand(WPARAM wParam, LPARAM lParam)
{
	// TODO: Add your specialized code here and/or call the base class
...
void CInfoBar::AddButton(CString& text, int CtrlID, int Type)
{
	sCtrlButton* pButton = new sCtrlButton(text, CtrlID, Type);
	mButtonsList.AddTail(pButton);
}

void CInfoBar::AddIcon(int IconID, int CtrlID)
{
	sCtrlButton* pButton = new sCtrlButton(CString(_T("")), CtrlID, BT_ICON);
	pButton->iIconID = IconID;

	mButtonsList.AddTail(pButton);
}

void CInfoBar::ReleaseButtons()
{
	while (mButtonsList.GetCount() > 0)
	{
		sCtrlButton* pButton = mButtonsList.GetHead();
		mButtonsList.RemoveHead();

		delete pButton;
	}
}

void CInfoBar::ShowButtons(Graphics* pGraphics)
{	
	int iStartOffset = CalculateStartButtonsOffset();

	for (POSITION pos = mButtonsList.GetHeadPosition();pos;)
	{
		sCtrlButton* pButton = mButtonsList.GetNext(pos);
		
		ShowButton(pGraphics, pButton, iStartOffset);		
		
		iStartOffset += CalculateButtonLength(pButton);
	}
}

int CInfoBar::GetButtonOffset(sCtrlButton* pButton)
{
	int iOffset = CalculateStartButtonsOffset();

	for (POSITION pos = mButtonsList.GetHeadPosition();pos;)
	{
		sCtrlButton* pCurButton = mButtonsList.GetNext(pos);

		if (pCurButton == pButton)
			break;	
		
		iOffset += CalculateButtonLength(pCurButton);
	}

    return iOffset;
}

void CInfoBar::ShowButton(Graphics* pGraphics, sCtrlButton* pButton, int Offset)
{
	CRect ClRect;
	GetClientRect(&ClRect);

	if (Offset + CalculateButtonLength(pButton) > (ClRect.right - 35))
		return;

	if (pButton->iButtonType == BT_STANDART_BUTTON || 
		pButton->iButtonType == BT_DROPDOWN_MENU_BUTTON)
	{
		if (pButton == pActiveButton)
		{
			int iButtonLength = CalculateButtonLength(pButton);

			Color ButtonTopGradColor(255, 244, 204);
			Color ButtonBottomGradColor(255, 214, 154);		

			LinearGradientBrush ButtonGradBrush(
				Point(0, 3),
				Point(0, ClRect.bottom - 4),
				ButtonTopGradColor,   
				ButtonBottomGradColor);

			pGraphics->FillRectangle(&ButtonGradBrush, 
				Offset + 1, 3,
				iButtonLength - 1, ClRect.bottom - 7);	

			Color BorderColor(75, 75, 111);
			Pen BorderPen(BorderColor);

			pGraphics->DrawRectangle(&BorderPen, Offset, 2, 
				iButtonLength, ClRect.bottom - 6);
		}
		...
		PointF      point(Offset + IB_BUTTON_MARGIN - 1, 5);
		SolidBrush  TestColorBrush(Color(40, 13, 50));
		pGraphics->DrawString(T2W(pButton->csText), -1, &font, point, &TestColorBrush);
	} else if (pButton->iButtonType == BT_SEPARATOR)
	{		
		Pen SeparatorPen(Color(106, 140, 203), 1);	
		Pen	SeparatorShadowPen(Color(241, 249, 255), 1);

		pGraphics->DrawLine(&SeparatorPen, Offset + 3, 5, Offset + 3, 18);
		pGraphics->DrawLine(&SeparatorShadowPen, Offset + 4, 6, Offset + 4, 19);	
	} else if (pButton->iButtonType == BT_ICON)
	{
		if (pButton == pActiveButton)
		{
			int iButtonLength = CalculateButtonLength(pButton);
			...
			pGraphics->FillRectangle(&ButtonGradBrush, 
				Offset + 1, 2,
				iButtonLength - 1, ClRect.bottom - 5);	

			Color BorderColor(75, 75, 111);
			Pen BorderPen(BorderColor);

			pGraphics->DrawRectangle(&BorderPen, Offset, 1, 
				iButtonLength, ClRect.bottom - 4);
		} 

		HICON hIcon = LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(pButton->iIconID));
		
		HDC hDC = pGraphics->GetHDC();
		DrawIconEx(hDC, Offset + 3, 3, hIcon, 17, 17, 0, NULL, DI_NORMAL | DI_COMPAT/* | DI_DEFAULTSIZE*/);
...
int CInfoBar::CalculateButtonLength(sCtrlButton* pButton)
{
	if (pButton->iButtonType == BT_STANDART_BUTTON || 
		pButton->iButtonType == BT_DROPDOWN_MENU_BUTTON)
	{
		RectF boundingBox = GetTestMetrics(pButton->csText, CString("Tahoma"), 11, FontStyleRegular, UnitPixel);

		return (boundingBox.GetRight() - boundingBox.GetLeft()) + IB_BUTTON_MARGIN + 2;
	} else if (pButton->iButtonType == BT_SEPARATOR)
	{
		return 7;
	} else if (pButton->iButtonType == BT_ICON)
...
int	CInfoBar::CalculateStartButtonsOffset()
{
	CString		tmBarText;
	GetWindowText(tmBarText);

	RectF boundingBox = GetTestMetrics(tmBarText, CString("Times New Roman"), 15, FontStyleBold, UnitPixel);

	return (boundingBox.GetRight() - boundingBox.GetLeft()) + 11 + IB_BUTTON_MARGIN;
}

sCtrlButton* CInfoBar::GetButtonUnderMouse(CPoint& point)
{
	CRect ClRect;
	GetClientRect(&ClRect);
	...
	{
		sCtrlButton* pButton = mButtonsList.GetNext(pos);
		int iLength = CalculateButtonLength(pButton);

		CRect ButtonRect(iStartOffset + 1, 3,
			iStartOffset + iLength - 1, ClRect.bottom - 3);

		if (ButtonRect.PtInRect(point))
			return pButton;
		
		iStartOffset += iLength;
	}

	return NULL;
}

RectF CInfoBar::GetTestMetrics(CString& Text, CString& FontName, int FontSize, FontStyle Style, Unit MetricsUnit)
{
	USES_CONVERSION;
	...
	Graphics grGraphics(hDC);
	FontFamily fontFamily(T2W(FontName));
	Font font(&fontFamily, FontSize, Style, MetricsUnit);

	grGraphics.MeasureString(T2W(Text), Text.GetLength(),
		&font, RectF(0, 0, 1000, 1000), &boundingBox);
	DeleteDC(hDC);
