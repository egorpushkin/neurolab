			...
			if (bCtrl && m_nCurrHyperlinkHot != -1)
			{
			#ifdef _BCGPEDIT_STANDALONE
				{
					::SetCursor(::LoadCursor(
						...
						MAKEINTRESOURCE(IDC_BCGBARRES_HAND)));
				}
			#else
				{
					::SetCursor(::LoadCursor(
...
int CBCGPEditCtrl::GetRowStart (int nRow, BOOL bSkipHidden) const
{
	...
	int nIdx = 0, nRowIdx = 0;
	for (nIdx = 0, nRowIdx = 0;  nRowIdx != nRow; /*nRowIdx++,*/ nIdx++)
	{
...
void CBCGPEditCtrl::OnDrawSideBar (CDC* pDC, CRect rect)
{
	...
	pDC->FillSolidRect (&rectBar, ::GetSysColor (COLOR_3DLIGHT));
...
void CBCGPEditCtrl::OnDrawLineNumbersBar (CDC* pDC, CRect rect)
{
	...
	COLORREF clrLineNumber = RGB (0, 130, 132);
	...
	COLORREF clrTextOld = pDC->SetTextColor (clrLineNumber);
									 ...
									 BOOL bReplaceEOL)
{
	...
	{
		int nSpaceCount = 0, nTextIdx = 0, nExpIdx = 0;
		for (nTextIdx = 0, nExpIdx = 0; nTextIdx < nTextLen; nTextIdx++, nExpIdx++)
		{
										   ...
										   BOOL bSetCaret)
{
		...
		(pUndoAction->m_dwActionType & g_dwUATDeleteData) && !bUndo)
	{
		...
		{
			CString strChar = CString(pUndoAction->m_strActionText [0]);
						...
						CString& strTag, BOOL bIsCharsList = FALSE)
{
	...
	CString strTagStart = CString(_T('<'));
	strTagStart += lpszTag;
	strTagStart += _T('>');
...
int CBCGPEditCtrl::GetLine (int nLine, LPTSTR lpszBuffer) const
{
	...
	if (nEndOffset > nStartOffset && nEndOffset <= m_strBuffer.GetLength ())
	{
		...
		_tcsncpy (lpszBuffer, (LPCTSTR) strLine, min (nLineLen, nMaxLength) );
...
BOOL CBCGPEditCtrl::OnUpdateLineNumbersMarginWidth (CDC* pDC)
{
    ...
    CSize size = GetStringExtent (pDC, CString(_T('8'), nDigits), nDigits);
